// La Calma Comunidad — Translation Engine v2.0
// Exchange rate: 1 USD ≈ 17.22 MXN (May 2026)

const EXCHANGE_RATE = 17.22;

const translations = {
  es: {
    nav: {
      home: 'Inicio', map: 'Ubicación', lifestyle: 'Estilo de Vida',
      press: 'Prensa', transparency: 'Transparencia', simulator: 'Simulador',
      financing: 'Financiamiento', faq: 'FAQ', contact: 'Contacto', lang_btn: 'EN'
    },
    hero: {
      badge: 'SOLO 2 LOTES DISPONIBLES · PROPIEDAD PRIVADA · BAJA DENSIDAD H1',
      title: 'La Calma Comunidad:',
      subtitle: 'Tu último lienzo de baja densidad en el epicentro del viento.',
      desc: 'Dos fracciones tipo Estate de 1,500 m² — las últimas disponibles. Acceso privado de 13 m, vistas protegidas a Isla Cerralvo. No es Tulum. Es mejor: el crecimiento controlado de La Ventana, BCS.',
      coord_label: 'UBICACIÓN',
      coord_val: '24.070988° N · 109.998741° O',
      cta_primary: 'Agendar Visita',
      cta_secondary: 'Conoce el Entorno'
    },
    stats: {
      lots_avail: 'Disponibles', lots_total: 'Fracciones', surface: 'm² c/u',
      access: 'm Acceso Privado', density: 'Densidad H1'
    },
    map: {
      title: 'Lotificación Oficial',
      subtitle: 'Fracción 4 y 5 — las únicas disponibles. El resto, vendido.',
      available: 'Disponible', sold: 'Vendido',
      lots: {
        l1: 'Fracción 1 · Vendida',
        l2: 'Fracción 2 · Vendida',
        l3: 'Fracción 3 · Vendida',
        l4: 'Fracción 4 · Disponible — 1,500 m²',
        l5: 'Fracción 5 · Disponible — 1,505 m²'
      }
    },
    area_guide: {
      title: 'Guía Interactiva del Entorno',
      subtitle: 'Esto no es un terreno perdido en el desierto. Esto es La Ventana — un ecosistema completo de deporte, familia, gastronomía y naturaleza.',
      map_title: 'Explora la Zona',
      sports_title: 'Deportes y Vida Activa',
      sports_desc: 'La Ventana es reconocida por Forbes como uno de los mejores destinos del mundo para kitesurf. El "túnel de viento" natural entre la Isla Cerralvo y la Sierra de La Laguna genera vientos térmicos de 15-30 nudos de noviembre a abril.',
      sports_calendar: 'Calendario Deportivo',
      season_1_title: 'Temp. Alta Kitesurf',
      season_1_period: 'Noviembre — Abril',
      season_1_desc: 'Vientos térmicos 15-30 nudos. Competencias internacionales: Lighthouse 1000 (Ene), La Ventana Classic (Feb), Grom Fest (Dic). Escuelas de kite, renta de equipo.',
      season_2_title: 'Temp. de Ballenas',
      season_2_period: 'Enero — Marzo',
      season_2_desc: 'Ballenas grises y jorobadas visitan la bahía. Tours de avistamiento. El programa de nado con orcas más importante de Latinoamérica opera aquí.',
      season_3_title: 'Ciclismo y Senderismo',
      season_3_period: 'Abril — Junio',
      season_3_desc: 'Floración del desierto. Rutas de MTB de clase mundial. El Reto Baja y Don Diablo Trail Run atraen competidores globales. Temperaturas ideales.',
      season_4_title: 'Pesca y Buceo',
      season_4_period: 'Julio — Octubre',
      season_4_desc: 'Blue Water World Cup (Jun) — torneo internacional de pesca submarina. Aguas cálidas ideales para buceo en arrecifes de Isla Cerralvo.',
      season_5_title: 'Retiro y Comunidad',
      season_5_period: 'Octubre — Diciembre',
      season_5_desc: 'Regreso de snowbirds. Festival del Viento (May). Temporada previa al pico de vientos. Clima perfecto, comunidad activa.',
      day_title: 'Un Día en La Ventana',
      morning: '🌅 Mañana (6-10am) — Kitesurf al amanecer. Los vientos térmicos comienzan temprano. Desayuno en Café Bar Km 0.',
      midday: '☀️ Mediodía (10am-2pm) — Paddle board en aguas tranquilas o buceo en Isla Cerralvo. Comida en Nómada El Sargento.',
      afternoon: '🌤️ Tarde (2-6pm) — Siesta, lectura frente al mar o MTB en senderos del cerro. Café en Mare Spa.',
      night: '🌙 Noche (6-11pm) — Atardecer en el malecón de La Ventana. Cena gourmet. Cielo estrellado sin contaminación lumínica.',
      family_title: 'Logística Familiar',
      family_desc: 'Escuela Montessori bilingüe a 5 min. Farmacias locales. La Paz (hospitales de especialidad) a 35 min por carretera escénica. Starlink para trabajo remoto. Comunidad internacional activa de expatriados.',
      market_title: '¿Para quién es La Calma?',
      market_q: 'Para alguien que ya vivió Tulum, Puerto Escondido o Sayulita — y sabe lo que NO quiere: saturación, densidad, pérdida de autenticidad.',
      market_a: 'La Calma es para el inversionista que entiende que el verdadero lujo es la escasez. 5 fracciones. Solo 2 quedan. Sin blocks de condominios. Sin ruido. Sin gentrificación masiva. Esto es baja densidad controlada — el modelo que Tulum debió haber seguido y no siguió.',
      market_a2: 'Es para la familia que busca seguridad, naturaleza y comunidad. Para el nómada digital que necesita Starlink y silencio. Para el deportista que quiere vivir donde otros vacacionan.'
    },
    press: {
      title: 'Lo que Dicen de La Ventana',
      subtitle: 'Noticias reales que respaldan tu decisión de inversión.',
      articles: [
        {
          source: 'Forbes',
          date: 'Enero 2026',
          quote: '"La Ventana es un destino consentido para el kitesurf. La revista recomienda visitar La Paz y su malecón, y hace un énfasis particular en La Ventana como destino de clase mundial para deportes de viento."',
          url: 'https://www.posta.com.mx/bcs/forbes-destaca-a-la-ventana-como-un-destino-consentido-para-el-kitesurf/vl2148446',
          tag: 'Reconocimiento Internacional'
        },
        {
          source: 'SETUE BCS',
          date: 'Enero 2026',
          quote: '"La Paz alcanzó 668,800 pasajeros aéreos en 2025, con un crecimiento del 10.9% anual. Entre 2021 y 2025, la región recibió 20.9 millones de pasajeros — cifra histórica."',
          url: 'https://elmundodigital.com.mx/la-paz-destaca-por-dinamismo-turistico-trafico-aereo-en-bcs-crece-2-en-2025/',
          tag: 'Crecimiento Turístico'
        },
        {
          source: 'Canirac BCS',
          date: 'Enero 2026',
          quote: '"La Paz tuvo 17% más turistas al cierre de 2025 comparado con 2024. Se proyectan de 3 a 5 cruceros por mes durante 2026, generando derrama económica significativa."',
          url: 'https://www.bcsnoticias.mx/hubo-17-mas-turistas-en-la-paz-al-cierre-del-2025-que-en-2024-consejera-de-canirac/',
          tag: 'Derrama Económica'
        },
        {
          source: 'SEMARNAT',
          date: 'Agosto 2025',
          quote: '"Se aprobó el primer Plan de Manejo para observación y nado con orcas en Latinoamérica, operando en El Sargento, La Ventana y Agua Amarga. Atracción turística única en el mundo."',
          url: 'https://www.diarioelindependiente.mx/2025/08/aprueban-programa-de-observacion-turistica-de-orcas-en-el-sargento-la-ventana-y-agua-amarga',
          tag: 'Ecoturismo Regulado'
        },
        {
          source: 'Blue Water World Cup',
          date: 'Junio 2026',
          quote: '"20° aniversario del torneo internacional de pesca submarina en Palapas Ventana. Competidores de todo el mundo. $3,000 USD donados a la comunidad local para parques públicos."',
          url: 'https://www.palapasventana.com/blue-water-world-cup-2026/',
          tag: 'Evento Internacional'
        }
      ]
    },
    growth: {
      title: 'Proyección de Crecimiento',
      subtitle: 'La Ventana está donde Tulum estaba hace 15 años — antes de la saturación. Esto es crecimiento controlado con visión de futuro.',
      timeline: [
        { year: '2025', event: 'Crecimiento del 10.9% en tráfico aéreo de La Paz. Forbes destaca La Ventana como destino mundial de kitesurf.' },
        { year: '2026', event: '120,000 cruceristas programados (+27%). Programa de observación de orcas aprobado. Blue Water World Cup 20° aniversario.' },
        { year: '2027', event: 'Proyección: consolidación de La Ventana como hub de deportes de viento. Incremento de rutas aéreas directas a La Paz.' },
        { year: '2028-2030', event: 'Zona en franca plusvalía. Los lotes de baja densidad serán aún más escasos. El modelo H1 protege el valor a largo plazo.' }
      ],
      keystone: 'Dato clave: La Ventana-El Sargento cuenta con 22 hoteles, 283 habitaciones, y está superando la estacionalidad turística.',
      flora_title: 'Flora y Fauna',
      flora_desc: 'El desierto costero de Baja alberga cardones, biznagas, pitahayas y palo verde. La bahía es santuario de ballenas grises, jorobadas, orcas, delfines, lobos marinos y tortugas marinas. En temporada, el avistamiento de ballenas es diario desde la costa.',
      soil_title: 'Suelo y Construcción',
      soil_desc: 'Terreno firme de desierto con capa superficial arenosa y base de arena compactada — ideal para construcción. No hay riesgo de inundación (elevación natural). Factibilidad de CFE. Se recomienda biodigestor y cisterna.',
      density_title: '¿Qué se puede construir?',
      density_desc: 'Uso de suelo H1 (Habitacional Turística de Baja Densidad). Residencias unifamiliares de hasta 2 niveles, villas de descanso, complejos boutique. Altura máxima sugerida: 8 m. Prohibido: alta densidad, desarrollos masivos.'
    },
    transparency: {
      title: 'Transparencia Técnica Total',
      subtitle: 'Sin marketing vacío. Esto es lo que necesitas saber de un desarrollador que no te oculta nada.',
      water_title: 'Agua y Energía',
      water_body: 'La Ventana promueve crecimiento consciente. No dependemos de redes municipales sobrecargadas. Los propietarios implementan: cisternas de almacenamiento, biodigestores para tratamiento de aguas, y sistemas solares fotovoltaicos. Esto no es una limitación — es control total sobre tus servicios.',
      legal_title: '100% Propiedad Privada',
      legal_body: 'Cada fracción está regularizada ante Catastro de BCS con subdivisión autorizada. No es ejidal, no es concesión. Escrituras individuales transferibles. Sin riesgos de titulación.',
      density_title: 'Protección H1',
      density_body: 'Clasificación H1 (Habitacional Turística de Baja Densidad). Solo 5 fracciones. Nadie construirá un bloque que bloquee tu vista. Esa protección es jurídica.',
      foda_title: 'FODA del Desarrollador',
      foda_f: ['Solo 2 fracciones disponibles — escasez real', '1,500 m² con frentes de 44 m promedio', 'Vistas protegidas por topografía escalonada', '$56.66 USD/m² — por debajo del mercado ($70-110 USD/m²)'],
      foda_o: ['Forbes y medios internacionales posicionan La Ventana', '+10.9% tráfico aéreo anual en La Paz', 'Renta vacacional premium: $200-500 USD/noche', 'Escasez de lotes amplios cerca del núcleo deportivo'],
      foda_d: ['Requiere sistemas sustentables (agua/luz)', 'Solo 2 lotes disponibles — oferta limitada', 'Estacionalidad de viento para pico de rentas'],
      foda_a: ['Incremento en costos de construcción en BCS', 'Posibles desarrollos colindantes de mayor densidad', 'Cambios regulatorios ambientales']
    },
    simulator: {
      title: 'Simulador de Inversión',
      subtitle: 'Proyecta tu inversión total, construcción y retorno potencial con datos reales del mercado de La Ventana.',
      project_label: 'Tipo de Proyecto',
      project_villa: 'Villa Boutique (2-3 recámaras)',
      project_estate: 'Residencia Premium (4-5 recámaras)',
      surface_label: 'Superficie de Construcción',
      land_label: 'Fracción',
      frac4: 'Fracción 4 — $85,000 USD',
      frac5: 'Fracción 5 — $87,000 USD',
      build_cost: 'Costo de Construcción',
      build_m2: 'por m²',
      total_project: 'Inversión Total',
      land_title: 'Terreno',
      build_title: 'Construcción',
      rental_title: 'Proyección Airbnb',
      rental_note: 'Basado en datos reales de propiedades comparables en La Ventana',
      nightly_rate: 'Tarifa por noche (temp. alta)',
      occupancy: 'Ocupación promedio (6 meses)',
      monthly_low: 'Ingreso mensual (temp. baja)',
      annual_income: 'Ingreso Anual Estimado',
      annual_expenses: 'Gastos operativos (30%)',
      net_income: 'Ingreso Neto Anual',
      roi: 'ROI Neto Anual',
      disclaimer: '*Basado en promedios del mercado 2025-2026. Consulta con arquitectos y asesores locales.',
      cta: 'Solicitar Proyección Personalizada'
    },
    financing: {
      title: 'Financiamiento Directo: Sin Bancos, Sin Buró',
      subtitle: 'El mismo desarrollador te financia. Sin historial crediticio, sin aprobación bancaria, sin intereses ocultos.',
      down_val: '20%',
      down_amount: '$17,000 USD',
      term_val: '60 Meses',
      monthly: 'Pago Mensual Fijo',
      monthly_val: '$1,134 USD',
      monthly_mxn: '~$19,527 MXN/mes',
      total_paid: 'Pago Total',
      total_paid_val: '$85,000 USD',
      highlight_1: 'Precio fijo en USD desde el día 1. Sin ajustes inflacionarios.',
      highlight_2: 'Al firmar, puedes iniciar limpieza y planeación arquitectónica.',
      highlight_3: 'Respaldado por contrato de compraventa ante notario.',
      cta: 'Solicitar Contrato'
    },
    faq: {
      title: 'Preguntas que un Comprador Inteligente Hace',
      subtitle: 'Las respuestas que necesitas antes de invertir. Sin rodeos.',
      q1: '¿Por qué debería invertir en La Ventana y no en Tulum o Puerto Escondido?',
      a1: 'Porque Tulum ya se saturó. Puerto Escondido está en el mismo camino. La Ventana representa la última frontera de crecimiento controlado en BCS. La densidad H1 garantiza que nunca habrá desarrollos masivos. Los precios del suelo aún están por debajo del mercado ($56.66 vs $70-110 USD/m²). El tráfico aéreo de La Paz crece al 10.9% anual. Es timing perfecto.',
      q2: '¿Cuál es el estatus legal real?',
      a2: '100% Propiedad Privada. Regularizado ante Catastro de BCS. Subdivisión oficial autorizada. Cada fracción tiene su propio folio de registro. No es ejidal. No es concesión. Escrituras transferibles.',
      q3: '¿Puedo construir antes de terminar de pagar?',
      a3: 'Sí. Al firmar contrato y cubrir el enganche del 20%, puedes iniciar limpieza, nivelación y proyecto arquitectónico. No esperes a liquidar.',
      q4: '¿Hay cuotas ocultas?',
      a4: 'No. Mantenimiento del camino de acceso (13 m) se acuerda cooperativamente entre los 5 propietarios. Sin administradoras, sin inflación.',
      q5: '¿Hay agua y electricidad?',
      a5: 'Factibilidad de CFE. Para agua: cisterna de almacenamiento. Se recomienda biodigestor y paneles solares para autonomía total.',
      q6: '¿Puedo rentar en Airbnb?',
      a6: 'Sí. La Ventana es destino global de kiteboarding. Propiedades comparables: $200-500 USD/noche en temporada (Nov-Abr). Ocupación del 75-85% en vientos. Rentas de temporada baja: $2,500-4,500 USD/mes.',
      q7: '¿Hay escuelas, hospitales, servicios?',
      a7: 'Escuela Montessori bilingüe a 5 min. Farmacias locales. Hospitales especializados en La Paz a 35 min. Starlink disponible. Comunidad internacional activa.',
      cta_text: '¿Otra pregunta? Escríbenos'
    },
    cta: {
      title: 'Dos fracciones. Una decisión.',
      subtitle: 'Fracción 4 y Fracción 5 — las últimas.',
      desc: 'Bloquea el precio de $56.66 USD/m² antes del próximo ajuste. La Ventana no va a esperar.',
      btn: 'Agendar Llamada'
    },
    footer: {
      tagline: 'TRANQUILITY · BAJA LIFESTYLE',
      advisor: 'Ignacio Ibarra — Asesor Comercial',
      phone: '+52 (612) 107 7805',
      email: 'tranquilerealestate@gmail.com',
      rights: 'La Calma Community © 2026',
      location: 'La Ventana, BCS, México',
      privacy: 'Aviso de Privacidad', cookies: 'Política de Cookies', terms: 'Términos'
    }
  },

  // ========================== ENGLISH ==========================
  en: {
    nav: {
      home: 'Home', map: 'Location', lifestyle: 'Lifestyle',
      press: 'Press', transparency: 'Transparency', simulator: 'Calculator',
      financing: 'Financing', faq: 'FAQ', contact: 'Contact', lang_btn: 'ES'
    },
    hero: {
      badge: 'ONLY 2 LOTS AVAILABLE · PRIVATE PROPERTY · H1 LOW DENSITY',
      title: 'La Calma Comunidad:',
      subtitle: 'Your last low-density canvas at the epicenter of the wind.',
      desc: 'Two Estate-type fractions of 1,500 m² — the last available. Private 13m access road, protected views of Cerralvo Island. Not Tulum. Better: controlled growth in La Ventana, BCS.',
      coord_label: 'LOCATION',
      coord_val: '24.070988° N · 109.998741° W',
      cta_primary: 'Schedule Visit',
      cta_secondary: 'Explore the Area'
    },
    stats: {
      lots_avail: 'Available', lots_total: 'Fractions', surface: 'm² each',
      access: 'm Private Access', density: 'H1 Density'
    },
    map: {
      title: 'Official Subdivision',
      subtitle: 'Fraction 4 and 5 — the only ones available. The rest, sold.',
      available: 'Available', sold: 'Sold',
      lots: {
        l1: 'Fraction 1 · Sold',
        l2: 'Fraction 2 · Sold',
        l3: 'Fraction 3 · Sold',
        l4: 'Fraction 4 · Available — 1,500 m²',
        l5: 'Fraction 5 · Available — 1,505 m²'
      }
    },
    area_guide: {
      title: 'Interactive Area Guide',
      subtitle: 'This is not a lost plot in the desert. This is La Ventana — a complete ecosystem of sports, family, gastronomy and nature.',
      map_title: 'Explore the Zone',
      sports_title: 'Sports & Active Lifestyle',
      sports_desc: 'La Ventana is recognized by Forbes as one of the world\'s best kiteboarding destinations. The natural wind tunnel between Cerralvo Island and the Sierra de La Laguna generates thermal winds of 15-30 knots from November to April.',
      sports_calendar: 'Sports Calendar',
      season_1_title: 'Kitesurf High Season',
      season_1_period: 'November — April',
      season_1_desc: 'Thermal winds 15-30 knots. International competitions: Lighthouse 1000 (Jan), La Ventana Classic (Feb), Grom Fest (Dec). Kite schools, gear rental.',
      season_2_title: 'Whale Season',
      season_2_period: 'January — March',
      season_2_desc: 'Gray and humpback whales visit the bay. Whale watching tours. Latin America\'s most important orca swimming program operates here.',
      season_3_title: 'Cycling & Hiking',
      season_3_period: 'April — June',
      season_3_desc: 'Desert bloom. World-class MTB trails. El Reto Baja and Don Diablo Trail Run attract global competitors. Ideal temperatures.',
      season_4_title: 'Fishing & Diving',
      season_4_period: 'July — October',
      season_4_desc: 'Blue Water World Cup (Jun) — international spearfishing tournament. Warm waters ideal for diving at Cerralvo Island reefs.',
      season_5_title: 'Retreat & Community',
      season_5_period: 'October — December',
      season_5_desc: 'Snowbirds return. Wind Festival (May). Pre-wind season. Perfect weather, active community.',
      day_title: 'A Day in La Ventana',
      morning: '🌅 Morning (6-10am) — Sunrise kitesurf. Thermal winds start early. Breakfast at Café Bar Km 0.',
      midday: '☀️ Midday (10am-2pm) — Paddle board in calm waters or diving at Cerralvo Island. Lunch at Nómada El Sargento.',
      afternoon: '🌤️ Afternoon (2-6pm) — Siesta, oceanfront reading or MTB on mountain trails. Coffee at Mare Spa.',
      night: '🌙 Night (6-11pm) — Sunset at La Ventana boardwalk. Gourmet dinner. Star-filled sky with zero light pollution.',
      family_title: 'Family Logistics',
      family_desc: 'Bilingual Montessori school 5 min away. Local pharmacies. La Paz (specialty hospitals) 35 min by scenic highway. Starlink for remote work. Active international expat community.',
      market_title: 'Who is La Calma for?',
      market_q: 'For someone who has already lived Tulum, Puerto Escondido or Sayulita — and knows exactly what they DON\'T want: saturation, density, loss of authenticity.',
      market_a: 'La Calma is for the investor who understands that true luxury is scarcity. 5 fractions. Only 2 left. No condo blocks. No noise. No mass gentrification. This is controlled low density — the model Tulum should have followed but didn\'t.',
      market_a2: 'It\'s for the family seeking safety, nature and community. For the digital nomad needing Starlink and silence. For the athlete wanting to live where others vacation.'
    },
    press: {
      title: 'What They Say About La Ventana',
      subtitle: 'Real news backing your investment decision.',
      articles: [
        {
          source: 'Forbes',
          date: 'January 2026',
          quote: '"La Ventana is a favored destination for kitesurf. The magazine recommends visiting La Paz and its boardwalk, with special emphasis on La Ventana as a world-class wind sports destination."',
          url: 'https://www.posta.com.mx/bcs/forbes-destaca-a-la-ventana-como-un-destino-consentido-para-el-kitesurf/vl2148446',
          tag: 'International Recognition'
        },
        {
          source: 'SETUE BCS',
          date: 'January 2026',
          quote: '"La Paz reached 668,800 air passengers in 2025, growing 10.9% annually. Between 2021-2025, the region received 20.9 million passengers — an all-time record."',
          url: 'https://elmundodigital.com.mx/la-paz-destaca-por-dinamismo-turistico-trafico-aereo-en-bcs-crece-2-en-2025/',
          tag: 'Tourism Growth'
        },
        {
          source: 'Canirac BCS',
          date: 'January 2026',
          quote: '"La Paz had 17% more tourists at the close of 2025 compared to 2024. 3 to 5 cruise ships per month projected for 2026, generating significant economic impact."',
          url: 'https://www.bcsnoticias.mx/hubo-17-mas-turistas-en-la-paz-al-cierre-del-2025-que-en-2024-consejera-de-canirac/',
          tag: 'Economic Impact'
        },
        {
          source: 'SEMARNAT',
          date: 'August 2025',
          quote: '"Latin America\'s first Management Plan for orca observation and swimming was approved, operating in El Sargento, La Ventana and Agua Amarga. A unique world-class eco-tourism attraction."',
          url: 'https://www.diarioelindependiente.mx/2025/08/aprueban-programa-de-observacion-turistica-de-orcas-en-el-sargento-la-ventana-y-agua-amarga',
          tag: 'Regulated Ecotourism'
        },
        {
          source: 'Blue Water World Cup',
          date: 'June 2026',
          quote: '"20th anniversary of the international spearfishing tournament at Palapas Ventana. Competitors from around the world. $3,000 USD donated to local community for public parks."',
          url: 'https://www.palapasventana.com/blue-water-world-cup-2026/',
          tag: 'International Event'
        }
      ]
    },
    growth: {
      title: 'Growth Projections',
      subtitle: 'La Ventana is where Tulum was 15 years ago — before saturation. This is controlled growth with a future-focused vision.',
      timeline: [
        { year: '2025', event: '10.9% growth in La Paz air traffic. Forbes highlights La Ventana as a world-class kitesurf destination.' },
        { year: '2026', event: '120,000 cruise passengers scheduled (+27%). Orca observation program approved. Blue Water World Cup 20th anniversary.' },
        { year: '2027', event: 'Projected consolidation of La Ventana as wind sports hub. Increase in direct flight routes to La Paz.' },
        { year: '2028-2030', event: 'Zone in clear appreciation. Low-density lots will become even scarcer. H1 model protects long-term value.' }
      ],
      keystone: 'Key fact: La Ventana-El Sargento has 22 hotels, 283 rooms, and is overcoming tourism seasonality.',
      flora_title: 'Flora & Fauna',
      flora_desc: 'The Baja coastal desert is home to cardón cacti, biznagas, pitahayas and palo verde trees. The bay is a sanctuary for gray whales, humpbacks, orcas, dolphins, sea lions and sea turtles. In season, whale sightings are daily from the shore.',
      soil_title: 'Soil & Construction',
      soil_desc: 'Firm desert terrain with sandy surface layer and compacted sand base — ideal for construction. No flood risk (natural elevation). CFE electrical feasibility. Biodigester and cistern recommended.',
      density_title: 'What can be built?',
      density_desc: 'H1 land use (Low-Density Tourist Residential). Single-family residences up to 2 levels, vacation villas, boutique complexes. Max suggested height: 8 m. Prohibited: high density, massive developments.'
    },
    transparency: {
      title: 'Full Technical Transparency',
      subtitle: 'No empty marketing. What you need to know from a developer hiding nothing.',
      water_title: 'Water & Energy',
      water_body: 'La Ventana promotes conscious growth. We don\'t rely on overloaded municipal grids. Owners implement: storage cisterns, biodigesters for wastewater treatment, and solar PV systems. This isn\'t a limitation — it\'s total control over your utilities.',
      legal_title: '100% Private Property',
      legal_body: 'Each fraction is regularized with the BCS Cadastre with authorized subdivision. Not ejidal, not a concession. Individual transferable deeds. No title risks.',
      density_title: 'H1 Protection',
      density_body: 'H1 classification (Low-Density Tourist Residential). Only 5 fractions. Nobody will build a block blocking your view. That protection is legal.',
      foda_title: 'Developer SWOT',
      foda_f: ['Only 2 fractions available — real scarcity', '1,500 m² with 44 m average fronts', 'Views protected by stepped topography', '$56.66 USD/m² — below market ($70-110 USD/m²)'],
      foda_o: ['Forbes & international media position La Ventana', '+10.9% annual air traffic in La Paz', 'Premium vacation rental: $200-500 USD/night', 'Scarcity of large lots near sports core'],
      foda_d: ['Requires sustainable systems (water/power)', 'Only 2 lots available — limited supply', 'Wind seasonality for peak rentals'],
      foda_a: ['Rising construction costs in BCS', 'Possible higher-density adjacent developments', 'Environmental regulatory changes']
    },
    simulator: {
      title: 'Investment Simulator',
      subtitle: 'Project your total investment, construction costs and potential returns using real La Ventana market data.',
      project_label: 'Project Type',
      project_villa: 'Boutique Villa (2-3 bedrooms)',
      project_estate: 'Premium Residence (4-5 bedrooms)',
      surface_label: 'Construction Surface',
      land_label: 'Fraction',
      frac4: 'Fraction 4 — $85,000 USD',
      frac5: 'Fraction 5 — $87,000 USD',
      build_cost: 'Construction Cost',
      build_m2: 'per m²',
      total_project: 'Total Investment',
      land_title: 'Land',
      build_title: 'Construction',
      rental_title: 'Airbnb Projection',
      rental_note: 'Based on real comparable property data in La Ventana',
      nightly_rate: 'Nightly rate (high season)',
      occupancy: 'Average occupancy (6 months)',
      monthly_low: 'Monthly income (low season)',
      annual_income: 'Estimated Annual Income',
      annual_expenses: 'Operating expenses (30%)',
      net_income: 'Net Annual Income',
      roi: 'Net Annual ROI',
      disclaimer: '*Based on 2025-2026 market averages. Consult with local architects and advisors.',
      cta: 'Request Personalized Projection'
    },
    financing: {
      title: 'Direct Financing: No Banks, No Credit Check',
      subtitle: 'The developer finances you directly. No credit history, no bank approval, no hidden interest.',
      down_val: '20%',
      down_amount: '$17,000 USD',
      term_val: '60 Months',
      monthly: 'Fixed Monthly Payment',
      monthly_val: '$1,134 USD',
      monthly_mxn: '~MX$19,527/mo',
      total_paid: 'Total Payment',
      total_paid_val: '$85,000 USD',
      highlight_1: 'Price fixed in USD from day 1. No inflation adjustments.',
      highlight_2: 'Upon signing, start clearing and architectural planning.',
      highlight_3: 'Backed by notarized purchase agreement.',
      cta: 'Request Contract'
    },
    faq: {
      title: 'Questions a Smart Buyer Asks',
      subtitle: 'The answers you need before investing. No fluff.',
      q1: 'Why should I invest in La Ventana instead of Tulum or Puerto Escondido?',
      a1: 'Tulum saturated. Puerto Escondido is on the same path. La Ventana represents the last frontier of controlled growth in BCS. H1 density guarantees no massive developments will ever appear. Land prices are still below market ($56.66 vs $70-110 USD/m²). La Paz air traffic grows at 10.9% annually. Perfect timing.',
      q2: 'What is the actual legal status?',
      a2: '100% Private Property. Regularized with BCS Cadastre. Officially authorized subdivision. Each fraction has its own registration folio. Not ejidal. Not a concession. Transferable deeds.',
      q3: 'Can I build before finishing payments?',
      a3: 'Yes. Upon signing and covering the 20% down payment, you can start clearing, leveling and architectural planning. No need to wait until full payment.',
      q4: 'Are there hidden fees?',
      a4: 'No. Maintenance of the common access road (13 m) is cooperatively agreed among the 5 owners. No administrators, no inflation.',
      q5: 'Is there water and electricity?',
      a5: 'CFE electrical feasibility. Water: storage cistern. Biodigester and solar panels recommended for total autonomy.',
      q6: 'Can I rent on Airbnb?',
      a6: 'Yes. La Ventana is a global kiteboarding destination. Comparable properties: $200-500 USD/night in season (Nov-Apr). 75-85% occupancy during wind season. Low-season monthly: $2,500-4,500 USD.',
      q7: 'Are there schools, hospitals, services?',
      a7: 'Bilingual Montessori school 5 min away. Local pharmacies. Specialty hospitals in La Paz 35 min away. Starlink available. Active international community.',
      cta_text: 'Have another question? Write us'
    },
    cta: {
      title: 'Two fractions. One decision.',
      subtitle: 'Fraction 4 and Fraction 5 — the last ones.',
      desc: 'Lock in the $56.66 USD/m² price before the next adjustment. La Ventana won\'t wait.',
      btn: 'Schedule a Call'
    },
    footer: {
      tagline: 'TRANQUILITY · BAJA LIFESTYLE',
      advisor: 'Ignacio Ibarra — Commercial Advisor',
      phone: '+52 (612) 107 7805',
      email: 'tranquilerealestate@gmail.com',
      rights: 'La Calma Community © 2026',
      location: 'La Ventana, BCS, Mexico',
      privacy: 'Privacy Notice', cookies: 'Cookie Policy', terms: 'Terms & Conditions'
    }
  }
};
